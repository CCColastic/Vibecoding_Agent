...
async def test_client_keeps_successful_response_when_usage_is_missing(monkeypatch):
    class Completions:
        async def create(self, **request):
            message = SimpleNamespace(model_dump=lambda **_: {"content": "Answer"})
            return SimpleNamespace(
    ...
    monkeypatch.setenv("BASE_URL", "https://example.test")
    monkeypatch.setattr(model_module, "AsyncOpenAI", SDK)
    response = await model_module.DeepSeekClient(
        ModelConfig(model="test-model")
    ).llm_call(purpose="chat", messages=[], tools=[])
